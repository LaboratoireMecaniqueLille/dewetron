  ...
  def __init__(self, port, channels, baud=9600, reset_addresses=False):
    ...
    if reset_addresses or not self.check_addresses():
      ...
      while not self.check_addresses():
        ...
        if input("Retry ?([y]/n)> ").lower() in ["", "y", "o"]:
          self.set_addresses()
        else:
          break
  ...
  def check_addresses(self):
    for m in self.mod:
      if not m.check_address(self.ser):
        ...
        return False
    print("Addresses are correct! This does not necessarily mean their "
          "address corresponds to the number on the rack !! "
          "(Use reset_addresses=True to be sure of that!)")
    return True

  def set_addresses(self, reset=False):
    """This method is used to set the addresses of each module.
    if reset=True, all of the addresses will be reassigned,
    even if they seem correct.
    Else, They will only be reassigned if the module type is different
    Be sure to use reset=True when the modules have been replaced/moved.
    """
    a = 0
    print("Setting addresses!")
    print("The button on the corresponding module must be pressed when "
          "setting the address")
